...
| Windows | Character Map (`charmap.exe`), built into Windows | Inline `windows-third-party` CI script (`.github/workflows/ci.yml`) | `windows-latest`, GitHub-hosted | Window wait, elements/accessibility read, semantic clicks by accessible name and role, and live-value verification via clipboard state change | Capture, focus as a distinct verified step, typing, drag, window lookup and enumeration, latency, and flake rate | 1 | Passed once: button roles were present, Select and Copy were each clicked twice, and the clipboard changed from its sentinel to a non-empty value. Flake rate has not yet been measured. | [run 33338511206](https://github.com/crafter-agents/cuse/actions/runs/33338511206) |
